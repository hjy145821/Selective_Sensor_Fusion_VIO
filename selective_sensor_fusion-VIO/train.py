...
def test(args, test_loader, feature_ext, rec_feat, rec_imu, pose_net,
    ...
    for i, (imgs, imus, poses) in enumerate(test_loader):
        ...
        file_name = r'E:\SLAM\results\vo\result_seq' + str(k) + '_' + str(epoch) + '.csv'
        np.savetxt(file_name, result, delimiter=',')

        file_name = r'E:\SLAM\results\vo\truth_pose_seq' + str(k) + '_' + str(epoch) + '.csv'
        np.savetxt(file_name, truth_pose, delimiter=',')

        file_name = r'E:\SLAM\results\vo\truth_euler_seq' + str(k) + '_' + str(epoch) + '.csv'
...
def compute_trans_pose(ref_pose, tgt_pose):
    ...
    # trans_pose = np.linalg.invu(ref_pose[:, :, :3]) @ tmp_pose
