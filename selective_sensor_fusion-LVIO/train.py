...
from models import FlowNet, RecFeat, PoseRegressor, RecImu, Fc_Flownet, Hard_Mask, Soft_Mask, Hard_Mask_PC
...
def main():
    ...
    degradation_mode = 0
    fusion_mode = 6
    # 0: vision only 1: direct 2: soft 3: hard 4: Lvio dirent 5: Lvio soft 6: Lvio hard
    # # set saving path
    ...
    for i, (imgs, imus, poses, pc_imgs) in enumerate(train_loader):
        ...
        for j in range(0, len(imgs)-1):
            ...
            with torch.no_grad():
                ...
                raw_feature_points = feature_ext(tgt_pc_img_var, ref_pc_img_var)
...
def test(args, test_loader, feature_ext, rec_feat, rec_imu, pose_net,
    ...
    for i, (imgs, imus, poses, pc_imgs) in enumerate(test_loader):
        ...
        file_name = r'E:\SLAM\results\Lvio\direct\result_seq' + str(k) + '_' + str(epoch) + '.csv'
        np.savetxt(file_name, result, delimiter=',')

        file_name = r'E:\SLAM\results\Lvio\direct\truth_pose_seq' + str(k) + '_' + str(epoch) + '.csv'
        np.savetxt(file_name, truth_pose, delimiter=',')

        file_name = r'E:\SLAM\results\Lvio\direct\truth_euler_seq' + str(k) + '_' + str(epoch) + '.csv'
