...
class Hard_Mask_PC(nn.Module):
    def __init__(self, input_dim_img, input_dim_pc, output_dim):
        super(Hard_Mask_PC, self).__init__()

        self.input_dim_img = input_dim_img
        self.input_dim_pc = input_dim_pc
        self.output_dim = output_dim

        self.fc_img = nn.Linear(self.input_dim_img, self.output_dim)
        self.fc_pc = nn.Linear(self.input_dim_pc, self.output_dim)

    def forward(self, feat_img, feat_pc, temp=None):
        feat_img = feat_img.view(-1, self.input_dim_img)
        feat_pc = feat_pc.view(-1, self.input_dim_pc)

        feat_img = self.fc_img(feat_img)
        feat_pc = self.fc_pc(feat_pc)

        feat_combined = feat_img + feat_pc  # 将图像和点云特征相加

        if temp is None:
            z = F.gumbel_softmax(feat_combined, hard=True, dim=1)
        else:
            z = F.gumbel_softmax(feat_combined, tau=temp, hard=True, dim=1)
